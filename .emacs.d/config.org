...
(defun set-fonts ()
  ...
  (let ((scale (- (/ 22.0 24) (expt 10 -10))))
    (setq face-font-rescale-alist `(("Noto Sans TC" . ,scale)
                                    ("Noto Sans SC" . ,scale)
                                    ("Noto Sans JP" . ,scale))))
  ;; default-text-properties '(line-spacing 0.1 line-height 1.25)
  ;; default-text-properties '(line-height 24))
...
(defun set-keys ()
  ...
  (global-set-key (kbd "C-x RET y") 'clipboard-yank)
  (global-set-key (kbd "C-x RET C-w") 'clipboard-kill-region)
  (global-set-key (kbd "C-x RET M-w") 'clipboard-kill-ring-save)
